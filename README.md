...
> **v1.8.2 note**, hardening release that focuses on three things: recorder relief for installs with high-frequency sensors (1 Hz Victron Cerbo, JK BMS, Ecowitt feeds and similar) so loading Helios no longer blocks every other Lovelace card reading the same entities for 30 to 100 seconds, camera pose + lock persistence to localStorage so the framing sticks across page reloads, and a chip family harmonisation pass (cloud chip joins the canonical recipe, leader-bead radii unified at 6 px, sun arc gets its own fullscreen ramp). The detail dashboard's today's battery totals are back to a full-day window after a beta over-trimmed them, and the back-to-live affordance is now a real tab anchored to the chart card. See [CHANGELOG](CHANGELOG.md) for the full beta-by-beta narrative, and the public roadmap with upcoming work tracked live on the GitHub Project at [**helios-lidar.org/roadmap**](https://helios-lidar.org/roadmap), refreshed every five minutes. Special thanks to **LBDG_** and **JJAsond** for the relentless field testing across the entire beta cycle on their 1 Hz installs, v1.8.2 would not be where it is without their patience.
