...
export const pt: Translations = {
    ...
    detail:
    {
        ...
        shadingDomeHint:      'Cúpula de sombreamento autoaprendida. Cada célula mostra a produção PV média nessa posição do sol, para a cobertura de nuvens escolhida abaixo. O Helios aplica-a à previsão para que as sombras reais (uma árvore às 16 h, uma chaminé no inverno) sejam capturadas automaticamente.',
    ...
    editor:
    {
        ...
        displayUpdateFrequencyHelp:   'Quantas vezes por hora cada gráfico atualiza os seus valores: 1 corresponde a um único ponto por hora (curva suave, CPU mínimo), 60 corresponde a um ponto por minuto (curva precisa, CPU máximo). Por omissão 4 = um valor a cada 15 minutos, um bom compromisso entre precisão visível e custo de renderização. A curva de previsão não é afetada: corre sempre na cadência horária nativa do modelo meteorológico.',
        ...
        inverterCutoffSocPctHelp:   'Percentagem à qual o seu inversor híbrido bloqueia a produção PV quando a bateria atinge o seu tecto configurado. Deixe vazio para desactivar. Quando definido, o treinador do mapa de sombreamento ignora todos os intervalos de observação onde o SoC da bateria atingiu este valor, para que a produção bloqueada pelo inversor não polua o mapa com sombras fantasma nas posições solares correspondentes.',
